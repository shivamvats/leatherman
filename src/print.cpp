    ...
    const std::string& text)
{
    ...
    for (unsigned int i = 0; i < traj.points.size(); i++) {
        ROS_INFO("[%s] [% 2d] % 1.4f % 1.4f % 1.4f % 1.4f % 1.4f % 1.4f % 1.4f   time_from_start: % 0.3f",
                text.c_str(), i, traj.points[i].positions[0],
    ...
    const std::string& text)
{
    for (unsigned int i = 0; i < points.size(); i++) {
        ROS_INFO("[%s] [% 2d] % 1.4f % 1.4f % 1.4f % 1.4f % 1.4f % 1.4f % 1.4f   time_from_start: % 0.3f",
                text.c_str(), i, points[i].positions[0], points[i].positions[1],
                points[i].positions[2], points[i].positions[3],
                points[i].positions[4], points[i].positions[5],
                points[i].positions[6], points[i].time_from_start.toSec());
    }
}
