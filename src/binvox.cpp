...
bool leatherman::convertBinvoxToOctomapMsg(
    const std::string& filename,
    double resolution,
    octomap_msgs::Octomap& msg)
{
    boost::shared_ptr<octomap::OcTree> octree(new octomap::OcTree(resolution));
    boost::shared_ptr<octomap::Pointcloud> point_cloud(
...
bool leatherman::convertBinvoxToPointCloudMsg(
    const std::string& filename,
    sensor_msgs::PointCloud& pc)
{
    ...
    pc.channels.clear();
    for (octomap::point3d_collection::iterator it = point_cloud->begin();
...
bool leatherman::convertBinvoxToVector3d(
    const std::string& filename,
...
bool leatherman::createBinvoxFile(
...
bool leatherman::convertBinvoxToBtSystem(
    const std::string& binvox_filename,
    std::string& bt_filename)
{
    int result = 1000;
    std::string path = leatherman::getPathWithoutFilename(binvox_filename);
    std::string filename = leatherman::getFilenameFromPath(
            binvox_filename, true);
    bt_filename = path + filename + ".bt";

    std::string command = "binvox2bt --mark-free -o " + bt_filename + " " +
            binvox_filename;
    try {
        result = system(command.c_str());
    }
...
bool leatherman::convertBinvoxToBt(
    const std::string& binvox_filename,
    std::string& bt_filename)
{
    ...
    std::ifstream *input = new std::ifstream(
            binvox_filename.c_str(), std::ios::in | std::ios::binary);
    if (!input->good()) {
        std::cerr << "Error: Could not open input file " << binvox_filename <<
                "!" << std::endl;
        exit(1);
    }
    else {
        std::cout << "Reading binvox file " << binvox_filename << "." <<
                std::endl;
    ...
    *input >> version;
        ...
        *input >> line;
        if (line.compare("data") == 0) {
            ...
            *input >> scale;
            ...
            std::cout << "    unrecognized keyword [" << line <<
                    "], skipping" << std::endl;
            char c;
    ...
    if (!tree) {
        std::cout << "Generating octree with leaf size " << res << std::endl <<
                std::endl;
    ...
    *input >> value;    // read the linefeed char
    ...
    input->close();
    std::cout << "    read " << nr_voxels << " voxels, skipped " <<
            nr_voxels_out << " (out of bounding box)\n\n";
...
void leatherman::convertOcTreeToCollisionMap(
...
void leatherman::getOccupiedVoxelsInOcTree(
...
bool leatherman::getOccupiedVoxelsInBinvoxFile(
    const std::string& binvox_filename,
    std::vector<Eigen::Vector3d>& voxels)
{
    int version;               // binvox file format version (should be 1)
    ...
    *input >> version;
    ...
    int maxSide = std::max(std::max(width, height), depth);
    double res = double(scale) / double(maxSide);
    ROS_ERROR("size: %d  width: %d  depth: %d  height: %d  maxSide: %d   res:  %f",
            size, width, depth, height, maxSide, res);
    if (!tree) {
        std::cout << "Generating octree with leaf size " << res << std::endl <<
                std::endl;
        tree = new octomap::OcTree(res);
